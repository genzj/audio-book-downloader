...
# Usage: ./cut-video.sh time-table.txt input-file.mp3
...
process_segment() {
    ...
    local output_file="${id}-${sanitized_title}.${extension}"
...
main() {
    # Check arguments
    if [[ $# -ne 2 ]]; then
        print_error "Usage: $0 <time-table.txt> <input-file.mp3|mp4>"
        exit 1
    fi
    ...
    for i in {1..${#segments[@]}}; do
        ...
        process_segment "$id" "$title" "$start_time" "$end_time" "$input_file" "$extension" &
        pids+=($!)
        ((job_count++)) || true
    done
